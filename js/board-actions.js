...
console.log('Checking positions', turn);
  if (game.in_checkmate()) {
    postEndGame();
    $('#game-state').text('Șah mat').removeClass('hidden');
  }

  else if (game.in_draw()) {
    postEndGame();
    $('#game-state').text('Jocul s-a încheiat in egal.').removeClass('hidden');
  }
    ...
    $('#game-state').text('Ești în șah!').removeClass('hidden');
  }

  else {
    if (turn == 'player') {
      $('#board').removeClass('locked');
      $('#game-turn').text('E rândul tau!');
      $('#game-state').text('E rândul tau!').removeClass('hidden');
    }

    if (turn == 'computer') {
      if(engineDisabled) {
        $('#board').removeClass('locked');
        $('#game-turn').text('E rândul adversarului!');
        $('#game-state').text('E rândul adversarului!').removeClass('hidden');
      } else {
        $('#board').addClass('locked');
        $('#game-turn').text('E rândul adversarului!');
        $('#game-state').text('E rândul adversarului!').removeClass('hidden');
